...
- **Documents dans le RAG** : à l'upload, le texte du document est extrait (`src/lib/subjects/extract-text.ts` — pdf via `unpdf`, docx/pptx/xlsx via `fflate`+XML, txt/csv natif), chunké (`chunkText`) et indexé dans `rag_chunks` (`source_type='subject_file'`, `metadata={subject_id,file_name}`) en arrière-plan (`after()` → `src/lib/rag/index-file.ts`). **Visibilité héritée du sujet** (confidentiel → `member`) et **resynchronisée** à chaque réindexation du sujet ; chunks purgés à la suppression du fichier/sujet. Consommé par (a) la **génération assistée** d'un champ (RPC `match_subject_files`, scopée au sujet, k=4) et (b) l'**assistant Astra** (via `match_rag_chunks`, filtré par tier → un doc de sujet confidentiel n'est jamais servi au visiteur), citations liées au sujet parent. Migration `011`. Indexation **non rétroactive** (au prochain upload).
