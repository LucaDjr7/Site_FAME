...
> 🟢 **REPRISE — état au 2026-06-24 (fin de session).** `main` = commit `ed5d951`, propre, à jour avec le remote (43 tests verts, `tsc`/`lint`/`build` à 0).
...
> - **⚠️ Bug systémique latent NON corrigé (décision : à traiter dans une passe dédiée)** : en Tailwind v4, `tailwind.config.ts` n'est pas chargé sans directive `@config` (absente de `globals.css`) → **toutes les classes `bg-fame-*` / `text-fame-*` / `border-fame-*` sont mortes (CSS non généré)**. Touche 9 fichiers (`auth/login`, `auth/activate`, `ui/StatusBadge`, `ui/EditModeToggle`, `ui/ConfirmDialog`, layouts `[lab]`/`admin`, `privacy`, `layout/LanguageSwitcher`). Le reste de l'app utilise des styles inline (hex) donc « passe » visuellement. **Contournement appliqué localement** (valeurs arbitraires `bg-[#hex]` qui, elles, génèrent toujours). **Vrai fix** = ajouter `@config "../../tailwind.config.ts"` (ou migrer les tokens en `@theme`) dans `globals.css`, MAIS ça réactive toutes ces classes d'un coup → re-vérif visuelle des 9 fichiers requise avant de merger. Ne PAS écrire de nouvelles classes `*-fame-*` Tailwind tant que ce n'est pas fait — utiliser inline hex ou `*-[#hex]`. Voir mémoire `tailwind-fame-tokens-dead`.
> - **Prochaines pistes ouvertes** (non démarrées) : domaines **D4–D7 de l'audit** (dette technique, i18n résiduel, a11y/SEO, config) → Vagues 2–4 à cadrer (brainstorming → spec → plan) ; **déploiement (Task 20)** ; le fix `@config` Tailwind ci-dessus.
...
| 2026-06-24 | **🐛 Bug systémique Tailwind v4 découvert (NON corrigé, à traiter en passe dédiée).** `tailwind.config.ts` n'est jamais chargé : il manque la directive `@config` dans `globals.css` (et pas de bloc `@theme`). → **toutes les classes `bg-fame-*`/`text-fame-*`/`border-fame-*` ne génèrent aucun CSS** (vérifié : `#c0473b` absent du CSS compilé avant fix). 9 fichiers concernés (cf. bloc Reprise). L'app « passe » car le reste est en styles inline hex. **Règle d'ici le fix : ne pas écrire de classes `*-fame-*` Tailwind ; utiliser inline hex ou `*-[#hex]`.** Vrai fix (à valider avec re-vérif visuelle des 9 fichiers) : ajouter `@config "../../tailwind.config.ts"` ou migrer les tokens en `@theme`. Voir mémoire `tailwind-fame-tokens-dead`. |
