...
- **Upload de fichiers sur fiche sujet** (branche `feat/subject-file-upload`) : dépôt direct (≤50 Mo, docs courants) en complément des liens Dropbox. Bucket privé `subject-files` + table `subject_files` (**migration `010` à appliquer en BDD**). Upload 3-temps signé (sign → upload direct navigateur → register), download via endpoint app (URL signée 60 s, gate `confidentiel`), suppression membres. Spec `docs/superpowers/specs/2026-06-28-subject-file-upload-design.md`, plan `docs/superpowers/plans/2026-06-28-subject-file-upload.md`. NB : la migration `010` désormais = subject_files ; les compteurs atomiques différés (audit M2/M3) prendront `011`.
- **Branches feature non mergées** : `fix/audit-2026-06-28` (audit), `fix/translate-keep-technical-terms` (prompts traduction/génération), `feat/subject-file-upload` (cette feature) — à merger après validation. Migrations `008`/`009` appliquées en BDD (2026-06-28). Prochaine étape : appliquer migration `010`, vérif navigateur, remplir le site.
