...
const ACTIVE: React.CSSProperties = { background: 'rgba(47,68,134,0.12)', border: '1px solid' }
const INACTIVE: React.CSSProperties = { background: 'transparent', border: '1px solid rgba(87,104,172,0.25)' }
    ...
    return (
      ...
      >
        <span className="font-mono text-fame-slate" style={{
          ...
          textTransform: 'uppercase', writingMode: 'vertical-rl', marginTop: 8,
        }}>
          {t('filters')}
        </span>
        {hasActive && <span className="bg-fame-blue" style={{ width: 7, height: 7, borderRadius: '50%', marginTop: 10 }} />}
      </div>
    )
  }
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 14px 12px' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {hasActive && (
            <button className="font-mono text-fame-slate" onClick={onReset} style={{
          ...
          <button className="font-mono text-fame-slate" onClick={onToggle} style={{  fontSize: 12, background: 'none', border: 'none', cursor: 'pointer' }}>
            »
          </button>
        </div>
      </div>
      ...
      {people.length > 0 && (
        <div style={{ padding: '0 14px 14px' }}>
          ...
          {people.map(m => {
            ...
            return (
              <button key={m.id} onClick={() => onTogglePerson(m.id)} aria-pressed={active} className={`font-mono ${active ? 'text-fame-blue border-fame-blue' : 'text-fame-text-muted'}`} style={{ ...btnBase, ...(active ? ACTIVE : INACTIVE) }}>
